...
modelData <- function(nInd, ID){
  ...
  Intensityday <- sapply(1:3,function(x){rnormt(nInd, range = c(0,Inf), mu = 9 / (1 + exp(-multim[1]*(Infectlvl-multim[2]))), 
                                                  s = interCCAsd[ID])})
...
modelSensSpecCCA <- function(nInd, ID){
  ...
  Intensityday <- sapply(1:3,function(x){c(rnormt((nInd-nInf), range = c(-3,Inf), mu = 0, s = interCCAsd[ID]),
                                           rnormt(nInf, range = c(0,Inf), mu = 9 / (1 + exp(-multim1_CCA[ID]*(Infectlvl-multim2_CCA[ID]))), 
                                                  s = interCCAsd[ID]))})
...
modelSensSpecREC <- function(nInd, ID){
  ...
  Intensityday <- sapply(1:3,function(x){c(rnormt((nInd-nInf), range = c(-3,Inf), mu = 0, s = interRECCCAsd[ID]),
                                           rnormt(nInf, range = c(0,Inf), mu = 9 / (1 + exp(-multim1_REC[ID]*(Infectlvl-multim2_REC[ID]))), 
                                                  s = interRECCCAsd[ID]))}) 
  mIntensity1 <- Intensityday[,1]
  mIntensity2 <- rowMeans(Intensityday[,1:2])
  mIntensity3 <- rowMeans(Intensityday)
